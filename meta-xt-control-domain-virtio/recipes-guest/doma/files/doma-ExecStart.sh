...
create_domain() {
    echo "Creating DomA"
    /usr/sbin/xl create /etc/xen/doma.cfg
    echo "Created DomA"
...
clean_up_domain() {
    if [ "$DOMA_ID" != "" ]; then
        echo "Cleaning up DomA domain with id ${DOMA_ID}"
        ...
        # it the second time.
        local DOMA_ID_LOCAL=$(xl list | awk '{ if ($1 == "DomA") print $2 }')

        local XL_DESTROY_PID=""

        if [ "$DOMA_ID_LOCAL" != "" ]; then
            echo "Destroying DomA"
            /usr/sbin/xl destroy DomA &
            XL_DESTROY_PID=$!
            echo "Destroyed DomA"
        else
            echo "Was not able to fetch DomA domain id. Skip destroying the domain."
        fi
        ...
        echo "Cleaned up DomA domain with id ${DOMA_ID}"
    else
        echo "No id of DomA available. Skip destroying and cleaning up the domain."
    fi
}
...
monitor_domain() {
    ...
    XS_PATH="/local/domain/$DOMD_ID/backend/qvirtio/${DOMA_ID}/0/state"
